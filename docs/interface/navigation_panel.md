...
??? info "Top row — left side: the mode toggles"
    ...
    | **Diff State** (bounding box) | Colours objects by the state they are in — keyed, or drifted from their rest baseline. Click it to switch it on or off. ++shift++ + click picks the state: **Take State**, **Drift State** or **All** — see [Diff State](../features/diff_state.md). It only looks at the scene, so nothing else ever holds it off. |
