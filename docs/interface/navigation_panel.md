...
??? info "Top row — left side: the mode toggles"
    ...
    | **Timeline Mode** | Chooses what the timeline drives. Click it to switch it on or off. ++shift++ + click picks the mode. ++alt++ + click opens the settings behind it. **Frame Sync** keeps every take on the same frame. **Scene Follow** lets the window move with your mouse between a sequencer and a viewport — see [Sequencer](../features/sequencer.md). Only Frame Sync is greyed out while Still Mode is on: the two pull the playhead in opposite directions. |
    ...
    | **Diff State** (bounding box) | Colours objects by the state they are in — keyed, or drifted from their rest baseline. Click it to switch it on or off. ++shift++ + click picks the marks: **Drifted Marks**, **Slot Marks** or **All** — see [Diff State](../features/diff_state.md). It only looks at the scene, so nothing else ever holds it off. |
