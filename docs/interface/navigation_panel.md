...
??? info "Top row — left side: the mode toggles"
    ...
    | **Diff State** (bounding box) | Colours objects by the state they are in — keyed, or drifted from their rest baseline. Click it for the four choices: **Drifted Marks**, **Slot Marks**, **All**, **Off**. They are exclusive, so picking one switches the others off — see [Diff State](../features/diff_state.md). It only looks at the scene, so nothing else ever holds it off. Hide this button in Preferences if you do not want it. |
