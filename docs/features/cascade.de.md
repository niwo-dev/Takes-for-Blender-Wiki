...
Die **Cascade** ist die Kern-Engine von Takes for Blender. Sie löst Eigenschaftsüberschreibungen über eine 6-stufige Hierarchie auf, wobei jede Ebene eine darüber liegende Ebene überschreiben kann.
...
```mermaid
graph LR
    G[Global] --> SG[Szenengruppe]
    SG --> S[Szene]
    S --> VLG[VL-Gruppe]
    VLG --> VL[Ansichtslayer]
    VL --> VLV[VL-Version]
...
| **Ansichtslayer** | Einzelner VL | Kamera, Aktion, Welt pro Aufnahme |
