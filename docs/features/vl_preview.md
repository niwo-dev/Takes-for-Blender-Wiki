...
- The operator currently has no permanent button in the UI — invoke it from Blender's operator search (++f3++) with the View Layer's scene name and view-layer name as parameters. A row-menu entry is planned.
