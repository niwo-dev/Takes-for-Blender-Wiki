---
icon: material/school
---

# A7 · When something looks wrong

4 lessons · 21 min · **The panel tells you what is going on**.

## 1. What the warning icons mean <span class="tks-min">5 <span class="tks-min__u">min</span></span> { #1-what-the-warning-icons-mean }
<div class="tks-video" data-video="A7.1">What the warning badges mean, and the fix button behind each</div>

Warning icons only appear when something is wrong, like an unsaved preset change or a missing preset file. Each icon is one family of warnings. Click it, pick a warning at the panel's top right, and you see the problem and the button that fixes it.

Read more: [Navigation Panel](../interface/navigation_panel.md#warnings)

## 2. Something looks wrong? Check this first <span class="tks-min">6 <span class="tks-min__u">min</span></span> { #2-something-looks-wrong-check-this-first }
<div class="tks-video" data-video="A7.2">My scene looks wrong - which tool to check first</div>

An object jumped, a value keeps moving, the timeline will not play. For each symptom, the one panel or setting to look at first.

Read more: [Troubleshooting](../troubleshooting.md)

## 3. Use Takes on a file you already have <span class="tks-min">5 <span class="tks-min__u">min</span></span> { #3-use-takes-on-a-file-you-already-have }
<div class="tks-video" data-video="A7.3">How to open an existing project in Takes: Keep It All or Decide One by One</div>

Open an older project and Takes asks one question about the animation, worlds and cameras it found. Keep It All leaves everything as it was. Decide One by One lets you sort each item.

Read more: [Navigation Panel](../interface/navigation_panel.md#adopted)

## 4. Panel locked or takes gone? How to recover <span class="tks-min">5 <span class="tks-min__u">min</span></span> { #4-panel-locked-or-takes-gone-how-to-recover }
<div class="tks-video" data-video="A7.4">Takes panel locked? Snapshots, the restore question and Rebuild Cache</div>

Takes keeps a safety copy of your groups, tags and rules on disk. If the tree goes flat or the panel locks, answer the restore question before you change anything.

Read more: [Recover Your Take Organisation](../workflows/recover_take_organisation.md)
