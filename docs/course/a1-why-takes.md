---
icon: material/school
---

# A1 · Why Takes exists

3 lessons · 14 min · the idea here is that **you branch instead of duplicating**.

## 1. The problem with saved copies <span class="tks-min">4 min</span> { #1-the-problem-with-saved-copies }
<div class="tks-video" data-video="A1.1">Stop saving scene_v7_FINAL.blend - one file, many looks</div>

Twelve files named after one shot. Only one is right and nobody remembers which. Takes keeps every round inside a single file.

Read more: [Takes vs Other Tools](../comparisons.md)

## 2. What Takes is for <span class="tks-min">4 min</span> { #2-what-takes-is-for }
<div class="tks-video" data-video="A1.2">What Takes for Blender does, and when not to use it</div>

It manages the stage: camera, world, materials, action and render settings. Many looks of one thing. It does not cut a sequence of shots.

Read more: [Takes vs Other Tools](../comparisons.md)

## 3. Who builds this, and why <span class="tks-min">6 min</span> { #3-who-builds-this-and-why }
<div class="tks-video" data-video="A1.3">Why I built Takes for Blender - a product artist's answer to variant chaos</div>

Where the problem comes from, what I tried before building anything, and how Takes is made. Your ideas steer where it goes next.

Read more: [Takes vs Other Tools](../comparisons.md) · [Roadmap](../roadmap/index.md)
