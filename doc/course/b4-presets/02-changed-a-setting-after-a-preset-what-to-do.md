# B4.2 · Changed a setting after a preset? What to do

**Stage B · Set up cameras, lights and looks** — B4 · Render presets · **5 min** · video 51 of 80

The idea this module plants: **Save render settings once, use them everywhere**.

## On YouTube

**Title**  
Changed a setting after applying a preset? Dirty state, Accept and Revert

**Description**  
The preset is now marked as changed. Accept saves your change into it, Revert puts the saved value back, and Alt+click updates every scene that uses it.

## What this video has to do

_Nothing yet — add what you know._

## What the viewer already knows

_Nothing yet — add what you know._

## What they can do afterwards

_Nothing yet — add what you know._

## What to show, in order

_Not planned yet._

## Set up before you record

_Nothing yet — add what you know._

## Where to be careful

_Nothing yet — add what you know._

## Out of scope

Name it and move on. Do not explain it here.

_Nothing yet — add what you know._

## Source of truth

The lesson links to: [Dirty State](../features/render_presets.md#dirty-state)

Check these before recording. If the page and the add-on disagree, the
page is wrong and gets fixed first.

## Open questions

_None._
