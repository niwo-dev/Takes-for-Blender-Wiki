# C2.3 · Render only some shots, or only the failed ones

**Stage C · Render and deliver** — C2 · Batch render: every shot at once · **4 min** · video 65 of 80

The idea this module plants: **One click renders every shot**.

## On YouTube

**Title**  
Render this scene, all scenes, by tag, resume, or retry failed

**Description**  
The render menu offers this scene, all scenes, one chosen scene, or one tag. Finished shots are skipped, so a stopped run picks up where it left off.

## What this video has to do

_Nothing yet — add what you know._

## What the viewer already knows

_Nothing yet — add what you know._

## What they can do afterwards

_Nothing yet — add what you know._

## What to show, in order

_Not planned yet._

## Set up before you record

_Nothing yet — add what you know._

## Where to be careful

_Nothing yet — add what you know._

## Out of scope

Name it and move on. Do not explain it here.

_Nothing yet — add what you know._

## Source of truth

The lesson links to: [The Render Menu](../features/batch_render.md#the-render-menu)

Check these before recording. If the page and the add-on disagree, the
page is wrong and gets fixed first.

## Open questions

_None._
